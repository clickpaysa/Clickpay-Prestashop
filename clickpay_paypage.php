...
/**
 * Clickpay - A 3rd party Payment Module for PrestaShop 1.6 & 1.7
 *
 * This file is the declaration of the module.
 *
 * @author Clickpay <https://Clickpay.com>
...
define('CLICKPAY_PAYPAGE_VERSION', '3.4.2');

require_once __DIR__ . '/clickpay_core.php';

function clickpay_error_log($msg, $severity)
{
    PrestaShopLogger::addLog($msg, $severity);
}

class Clickpay_PayPage extends PaymentModule
{
    private $_html = '';
    ...
    /**
     * Clickpay constructor.
     *
     * Set the information about this module
     */
    public function __construct()
    {
        $this->name                   = 'clickpay_paypage';
        $this->tab                    = 'payments_gateways';
        $this->version                = CLICKPAY_PAYPAGE_VERSION;
        $this->author                 = 'Clickpay';
        $this->controllers            = array('payment', 'validation', 'callback');
        $this->currencies             = true;
        $this->currencies_mode        = 'checkbox';
        $this->bootstrap              = true;
        $this->displayName            = 'Clickpay - PayPage';
    ...
    public function getContent()
    {
        ...
        $i = 2;
        $values = array_reduce(ClickpayApi::PAYMENT_TYPES, function ($acc, $method) use (&$i) {
            $code = $method['name'];
    ...
    protected function _postValidation()
    {
        ...
        foreach (ClickpayApi::PAYMENT_TYPES as $index => $method) {
            $code = $method['name'];
    ...
    public function hookPaymentOptions($params)
    {
         ...
         */
        // $paymentForm = $this->fetch('module:Clickpay_paypage/views/templates/hook/payment_options.tpl');
        ...
        foreach (ClickpayApi::PAYMENT_TYPES as $index => $payment) {
            $code = $payment['name'];
            $title = $payment['title'];
            $this->smarty->assign(['code' => $code]);

            if (!ClickpayHelper::paymentAllowed($code, $currency->iso_code)) continue;
            ...
            if ($code == 'all') {
                $title = 'Online payments powered by Clickpay';
                $desc = 'Online payments powered by Clickpay';
            } else {
                $desc = "Pay by Clickpay using $code";
            }
