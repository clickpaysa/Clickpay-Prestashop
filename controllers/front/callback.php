...
class Clickpay_PayPageCallbackModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if ($paymentKey === false) {
            PrestaShopLogger::addLog('Clickpay: Callback payment key is missing', 3, null, 'Cart', null, true, null);
            return;
        }

        $paymentType = ClickpayHelper::paymentType($paymentKey);
        ...
        if (!$result) {
            PrestaShopLogger::addLog('Clickpay: Callback reading response error', 3, null, 'Cart', null, true, null);
            return;
        }
        // $result = $ClickpayApi->verify_payment($paymentRef);
        ...
        if ($failed) {
            ...
            PrestaShopLogger::addLog(
                "Clickpay: Callback payment failed, payment_ref = {$transaction_ref}, response: [{$logMsg}]",
                2,
                null,
                'Cart',
                $orderId,
        ...
        if ($is_on_hold || $is_pending) {
            ...
            PrestaShopLogger::addLog(
                "Clickpay: Callback payment needs review, payment_ref = {$transaction_ref}, response: [{$res_msg}]",
                2,
                null,
                'Cart',
                $orderId,
        ...
        if (!$success) {
            $logMsg = json_encode($result);
            PrestaShopLogger::addLog(
                "Clickpay: Callback payment did not succeed, payment_ref = {$transaction_ref}, response: [{$logMsg}]",
                3,
                null,
                'Cart',
                $orderId,
        ...
        if (!$authorized) {
            ...
            PrestaShopLogger::addLog('Clickpay: Authorization error', 3, null, 'Cart', null, true, null);
        }
