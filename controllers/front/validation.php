...
class Clickpay_PayPageValidationModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (!$paymentRef || $paymentKey === false) {
            PrestaShopLogger::addLog('Clickpay: params error', 3, null, 'Cart', null, true, null);
        ...
        $result = $clickpayApi->read_response(false);
        if (!$result) {
            PrestaShopLogger::addLog('Clickpay: Return reading response error', 3, null, 'Cart', null, true, null);
            return;
        }
        ...
        if (!$success) {
            ...
            PrestaShopLogger::addLog(
                "Clickpay: payment failed, payment_ref = {$paymentRef}, response: [{$logMsg}]",
                3,
                null,
                'Cart',
                $orderId,
        ...
        if (!$authorized) {
            ...
            PrestaShopLogger::addLog('Clickpay: redirecting error', 3, null, 'Cart', null, true, null);
        }
